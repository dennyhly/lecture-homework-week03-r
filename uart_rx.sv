...
logic sync_ff1, sync_ff2, rx_sync, rx_ready_flag;

always @(posedge uart_clk) begin
    sync_ff1 <= rx_ready_flag;
    sync_ff2 <= sync_ff1;
    rx_sync <= sync_ff2;
    rx_ready <= sync_ff2 ^ rx_sync;
end
...
always @(posedge uart_clk) begin
    case (state)
        INIT: begin 
            state = IDLE; rx_counter <= 0; 
        end

        IDLE: begin 
            if (rx == 0) begin 
                state = RX_DATA; rx_counter <= 0;
            end
            else begin 
                state <= IDLE;
            end
        ...
        RX_DATA: begin
            rx_data[rx_counter] <= rx_sync;
            if (rx_counter < 7) begin
                rx_counter <= rx_counter + 1; state <= RX_DATA;
            end
            else begin 
                rx_counter <= 0; state <= STOP;
            end
        end

        STOP: begin
            if (rx_sync) begin
                state <= IDLE; rx_ready_flag <= ~rx_ready_flag;
            end
            else begin 
                state <= STOP;
            end
